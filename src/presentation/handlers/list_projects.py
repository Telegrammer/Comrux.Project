...
class ListProjectsHandler:
    ...
    async def __call__(self, raw_orders: str, offset: int, limit: int) -> list[ProjectRead]:
        response: list[ListProjectsElementResponse] = await self._usecase(ProjectListParams(
            OffsetPagination(offset, limit), self._orders_presenter(raw_orders)
        ))

        return [ProjectRead(title=elem["title"], description=elem["description"], created_at=elem["created_at"]) for elem in response]
