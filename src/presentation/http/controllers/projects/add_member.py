...
from presentation.handlers import AddProjectMemberHandler
...
def create_add_member_router() -> APIRouter:
    router = ErrorAwareRouter()

    @router.patch(
        "/{project_id}/add_member",
        error_map={
        ...
        response_model=ProjectMemberAdded,
    )
    @inject
    async def add_member(
        project_id: Annotated[UUID4, Path()],
        request_body: ProjectMemberAdd,
        handler: FromDishka[AddProjectMemberHandler],
